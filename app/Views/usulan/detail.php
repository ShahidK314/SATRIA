<?php 
// app/Views/usulan/detail.php (UPDATED: Remove Header Note & Split RAB Volume 6 Cols)
...
// FUNGSI UNTUK MEMECAH CATATAN BERDASARKAN SEKSI
...
<div class="m-5">
    <div class="mb-8">
        <a href="/pengajuan/usulan" class="text-slate-500 hover:text-blue-600 font-bold flex items-center gap-2 mb-4 transition-colors">
            <span class="material-icons text-sm">arrow_back</span> Kembali ke Daftar Usulan
        </a>
        <div class="flex justify-between items-start">
            <div>
                <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Detail Usulan Kegiatan</h1>
                <p class="text-slate-500 mt-1">Informasi lengkap KAK, IKU, dan RAB kegiatan.</p>
            </div>
            <div class="flex gap-2">
                <a href="/pdf/kak?id=<?php echo $usulan['id']; ?>" target="_blank" class="px-3 py-2 bg-rose-600 text-white font-bold rounded-lg hover:bg-rose-700 text-sm flex items-center" title="Cetak KAK">
                    <span class="material-icons text-lg mr-1">picture_as_pdf</span> KAK
                </a>
                <a href="/pdf/rab?id=<?php echo $usulan['id']; ?>" target="_blank" class="px-3 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 text-sm flex items-center" title="Cetak RAB">
    ...
    <div class="bg-white rounded-xl shadow-lg border border-slate-200 p-6 mb-8">
        <h2 class="text-2xl font-bold text-slate-800 mb-4"><?php echo htmlspecialchars($usulan['nama_kegiatan']); ?></h2>
        
        <div class="grid grid-cols-3 gap-4 border-t border-slate-100 pt-4">
            <div>
                <div class="text-xs font-bold text-slate-400 uppercase mb-1">Status Terkini</div>
                <span class="px-3 py-1 rounded-full text-sm font-bold <?php echo getStatusClass($usulan['status_terkini']); ?>">
                    <?php echo htmlspecialchars($usulan['status_terkini']); ?>
                </span>
            </div>
            <div>
                <div class="text-xs font-bold text-slate-400 uppercase mb-1">Total RAB (Pencairan)</div>
                <div class="text-lg font-bold text-emerald-600"><?php echo formatRupiah($usulan['nominal_pencairan']); ?></div>
            </div>
            <div>
                <div class="text-xs font-bold text-slate-400 uppercase mb-1">Kurun Waktu</div>
                <div class="text-sm text-slate-700"><?php echo date('d M Y', strtotime($usulan['tanggal_mulai'])); ?> - <?php echo date('d M Y', strtotime($usulan['tanggal_selesai'])); ?></div>
            </div>
        </div>
        
        <?php 
        // Logic ambil log terakhir untuk mapping catatan di bawah
        $latestLog = null;
        ...
        // --- BLOK RINGKASAN CATATAN DI HEADER DIHAPUS SESUAI REQUEST ---
        ?>
    </div>

    <?php
    // Persiapkan array catatan per bagian (KAK/IKU/RAB)
    $kakNotesArr = [];
    $ikuNotesArr = [];
    $rabNotesArr = [];
    if (!empty($latestLog)) {
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8">
        ...
        <div class="space-y-4 text-sm">
            <div class="flex items-start gap-4">
                ...
                <?php if ($note): ?>
                    <div class="w-64 text-xs text-rose-700 bg-rose-50 border border-rose-300 p-2 rounded-lg">
                        <div class="font-bold text-rose-800 mb-1">Catatan Revisi</div>
            ...
            <div class="flex items-start gap-4">
                <div class="flex-1">
                    <label class="text-xs font-bold text-slate-400 uppercase">Kurun Waktu</label>
                    <div class="text-sm text-slate-700"><?php echo date('d M Y', strtotime($usulan['tanggal_mulai'])); ?> - <?php echo date('d M Y', strtotime($usulan['tanggal_selesai'])); ?></div>
                </div>
            ...
            <div class="flex items-start gap-4">
                <div class="flex-1">
                    ...
                    <p class="text-slate-700 mt-1 bg-slate-50 p-3 rounded-lg max-h-40 overflow-y-auto"><?php echo nl2br(htmlspecialchars($usulan['gambaran_umum'])); ?></p>
                </div>
            ...
            <div class="flex items-start gap-4">
                <div class="flex-1">
                    ...
                    <p class="text-slate-700 mt-1 bg-slate-50 p-3 rounded-lg"><?php echo htmlspecialchars($usulan['penerima_manfaat']); ?></p>
                </div>
            ...
            <div class="flex items-start gap-4">
                <div class="flex-1">
                    ...
                    <p class="text-slate-700 mt-1 bg-slate-50 p-3 rounded-lg max-h-40 overflow-y-auto"><?php echo nl2br(htmlspecialchars($usulan['strategi_pencapaian_keluaran'])); ?></p>
                </div>
            ...
            <div class="flex items-start gap-4">
                <div class="flex-1">
                    ...
                    <ul class="list-disc list-inside text-slate-700 mt-1 bg-slate-50 p-3 rounded-lg">
            ...
            <div class="flex items-start gap-4">
                <div class="flex-1">
                    ...
                    <ul class="list-decimal list-inside text-slate-700 mt-1 bg-slate-50 p-3 rounded-lg">
            ...
            <div class="flex items-start gap-4">
                <div class="flex-1">
                    <label class="text-xs font-bold text-slate-400 uppercase">Indikator Kinerja & Bobot</label>
                    <table class="w-full text-xs mt-1 border border-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="p-2 border border-slate-200 w-6/12">Indikator</th>
                                <th class="p-2 border border-slate-200 text-center w-2/12">Bulan Target</th>
                                <th class="p-2 border border-slate-200 text-center w-1/12">Bobot (%)</th>
                                <th class="p-2 border border-slate-200 w-3/12">Catatan Revisi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr>
                                ...
                                <td class="p-2">
                                     ...
                                     <?php if ($note): ?>
                                         <div class="text-xs text-rose-700 bg-rose-50 border border-rose-300 p-1.5 rounded-lg">
                            ...
                            <?php else: ?>
                                <tr><td colspan="4" class="p-2 text-center text-slate-400 italic">Tidak ada indikator kinerja yang dicatat.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8">
        ...
        <?php else: ?>
            <table class="w-full text-sm border border-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="p-2 border border-slate-200 w-5/12">Deskripsi IKU</th>
                        <th class="p-2 border border-slate-200 text-center w-2/12">Target</th>
                        <th class="p-2 border border-slate-200 w-5/12">Catatan Revisi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8">
        <h3 class="text-xl font-bold text-slate-800 mb-4 flex items-center justify-between">
            <div class="flex items-center">
                <span class="material-icons text-amber-600 mr-2">payments</span> 
                RAB (Rencana Anggaran Biaya)
            </div>
        </h3>

        <?php if (empty($rabDetails)): ?>
            <p class="text-slate-400 text-sm italic">Tidak ada item RAB yang dicatat.</p>
        <?php else: ?>
                ...
                <table class="w-full text-sm border border-slate-200 whitespace-nowrap">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="p-2 border border-slate-200 text-left w-2/12">Catatan Revisi</th>
                        </tr>
                    </thead>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        ?>
                        <tr>
                            <td class="p-2 font-bold text-xs <?= $rab['nama_kategori'] !== $currentCat ? 'border-t-2 border-slate-300' : '' ?>">
                                <?= htmlspecialchars($rab['nama_kategori']) ?>
                            </td>

                            <td class="p-2 text-slate-700 <?= $rab['nama_kategori'] !== $currentCat ? 'border-t-2 border-slate-300' : '' ?>">
                            ...
                            <td class="p-2 text-center text-slate-700 <?= $rab['nama_kategori'] !== $currentCat ? 'border-t-2 border-slate-300' : '' ?>">
                                <?= $v1 > 0 ? $v1 : '-' ?>
                            </td>
                            <td class="p-2 text-center text-slate-700 <?= $rab['nama_kategori'] !== $currentCat ? 'border-t-2 border-slate-300' : '' ?>">
                                <?= $s1 ?>
                            ...
                            <td class="p-2 text-center text-slate-400 font-bold <?= $rab['nama_kategori'] !== $currentCat ? 'border-t-2 border-slate-300' : '' ?>">
                                <?= ($v2 > 0) ? 'x' : '' ?>
                            </td>
                            ...
                            <td class="p-2 text-right text-xs text-slate-600 <?= $rab['nama_kategori'] !== $currentCat ? 'border-t-2 border-slate-300' : '' ?>">
                                <?= formatRupiah($rab['harga_satuan']) ?>
                            </td>

                            <td class="p-2 text-right font-bold text-slate-700 <?= $rab['nama_kategori'] !== $currentCat ? 'border-t-2 border-slate-300' : '' ?>">
                        ...
                        <tr class="bg-amber-50 font-bold">
                            <td colspan="9" class="p-2 text-right uppercase">Total RAB :</td>
                            <td class="p-2 text-right text-lg text-amber-700"><?= formatRupiah($grandTotalRAB) ?></td>
                            <td class="p-2"></td>
                        </tr>
                    </tbody>
                </table>
