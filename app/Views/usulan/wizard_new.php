<?php include __DIR__.'/../partials/sidebar.php'; ?>
    ...
    <div class="m-5">
        <div class="mb-10 text-center">
            ...
            <p class="text-slate-500 mt-2">Lengkapi data substansi kegiatan (3 Langkah: KAK - IKU - RAB).</p>
        </div>

        <div class="flex items-center justify-center mb-12">
            <style>
                .step-indicator { display: flex; align-items: center; flex-direction: column; position: relative; z-index: 10; width: 100px; }
                .step-circle { width: 40px; height: 40px; border-radius: 50%; background-color: #e2e8f0; color: #64748b; display: flex; justify-content: center; align-items: center; font-weight: bold; transition: all 0.3s; border: 3px solid #e2e8f0; }
                .step-label { margin-top: 8px; font-size: 11px; font-weight: 600; text-align: center; color: #64748b; transition: color 0.3s; }
                ...
                .step-line { flex-grow: 1; height: 3px; background-color: #e2e8f0; margin: 0 -20px; transition: background-color 0.3s; }
        ...
        <form method="POST" action="<?php echo $actionUrl; ?>" id="usulanForm" class="bg-white p-8 rounded-xl shadow-lg border border-slate-200">
            ...
            <div class="step-content" data-step="1">
                <h2 class="text-2xl font-bold text-slate-800 mb-6">1. Detail KAK</h2>
                
                <div class="mb-6">
                    <label for="nama_kegiatan" class="block text-sm font-medium text-slate-700 mb-1">Nama Kegiatan </label>
                    <input type="text" id="nama_kegiatan" name="nama_kegiatan" value="<?php echo htmlspecialchars($usulan['nama_kegiatan'] ?? ''); ?>" required class="w-full px-3 py-2 border-slate-300 rounded-lg shadow-sm">
                </div>
                
                <div class="mb-6">
                    <label for="gambaran_umum" class="block text-sm font-medium text-slate-700 mb-1">Gambaran Umum</label>
                    <textarea id="gambaran_umum" name="gambaran_umum" rows="4" required class="w-full px-3 py-2 border-slate-300 rounded-lg shadow-sm"><?php echo htmlspecialchars($usulan['gambaran_umum'] ?? ''); ?></textarea>
                </div>
                
                <div class="grid grid-cols-2 gap-6 mb-6">
                    <div class="col-span-2">
                        <label for="penerima_manfaat" class="block text-sm font-medium text-slate-700 mb-1">Penerima Manfaat </label>
                        <input type="text" id="penerima_manfaat" name="penerima_manfaat" value="<?php echo htmlspecialchars($usulan['penerima_manfaat'] ?? ''); ?>" required class="w-full px-3 py-2 border-slate-300 rounded-lg shadow-sm">
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea id="strategi_pencapaian_keluaran" name="strategi_pencapaian_keluaran" rows="3" class="w-full px-3 py-2 border-slate-300 rounded-lg shadow-sm"><?php echo htmlspecialchars($usulan['strategi_pencapaian_keluaran'] ?? ''); ?></textarea>
                </div>
                
                <h3 class="font-bold text-slate-700 mb-3">Metode Pelaksanaan</h3>
                <div id="metode-container" class="space-y-3 mb-6">
                    <?php 
                    foreach ($metode_arr as $i => $metode): 
                        $hideDelete = ($i === 0 && count($metode_arr) === 1 && empty($metode));
                    ?>
                    <div class="flex gap-2 metode-item">
                        <input type="text" name="metode[]" value="<?php echo htmlspecialchars($metode); ?>" class="flex-1 px-3 py-2 border-slate-300 rounded-lg shadow-sm" placeholder="Contoh: Kuliah Umum / Workshop">
                ...
                <div id="tahapan-container" class="space-y-3 mb-6">
                    <?php 
                    foreach ($tahapan_arr as $i => $tahapan): 
                        $hideDelete = ($i === 0 && count($tahapan_arr) === 1 && empty($tahapan));
                    ?>
                    <div class="flex gap-2 tahapan-item">
                        <input type="text" name="tahapan[]" value="<?php echo htmlspecialchars($tahapan); ?>" class="flex-1 px-3 py-2 border-slate-300 rounded-lg shadow-sm" placeholder="Contoh: Pra-Acara: Pembuatan TOR">
                ...
                <div class="grid grid-cols-2 gap-6 mb-6 bg-slate-50 p-4 rounded-lg border border-slate-200">
                    <div>
                        ...
                        <input type="date" id="tanggal_mulai_kak" name="tanggal_mulai" value="<?php echo htmlspecialchars($usulan['tanggal_mulai'] ?? ''); ?>" required class="w-full px-3 py-2 border-slate-300 rounded-lg shadow-sm">
                    </div>
                    <div>
                        <label for="tanggal_selesai_kak" class="block text-sm font-medium text-slate-700 mb-1">Tanggal Selesai</label>
                        <input type="date" id="tanggal_selesai_kak" name="tanggal_selesai" value="<?php echo htmlspecialchars($usulan['tanggal_selesai'] ?? ''); ?>" required class="w-full px-3 py-2 border-slate-300 rounded-lg shadow-sm">
                    </div>
                </div>
                    ...
                    <tbody id="kpi-items" class="divide-y divide-slate-200">
                        ...
                        foreach ($indikator_arr as $kpi): ?>
                            <tr class="kpi-item" data-index="<?php echo $kpiIdx; ?>">
                                <td class="px-4 py-2">
                                    <input type="text" name="indikator_kinerja[<?php echo $kpiIdx; ?>][indikator]" value="<?php echo htmlspecialchars($kpi['indikator']); ?>" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm" placeholder="Contoh: Tercapainya 90% target peserta" required>
                                </td>
                                <td class="px-4 py-2">
                                    <select name="indikator_kinerja[<?php echo $kpiIdx; ?>][bulan_target]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm kpi-month-select" required>
                                ...
                                <td class="px-4 py-2">
                                    <input type="number" name="indikator_kinerja[<?php echo $kpiIdx; ?>][bobot]" value="<?php echo htmlspecialchars($kpi['bobot']); ?>" min="1" max="100" class="w-full text-center border-slate-300 rounded-md shadow-sm p-2 text-sm kpi-bobot-input" required>
                                </td>
                    ...
                    <tfoot class="bg-slate-50">
                        <tr>
                            <td colspan="2" class="px-4 py-2 text-right">
                                <button type="button" onclick="addKpiItem()" class="inline-flex items-center px-4 py-2 bg-emerald-600 text-white text-sm font-bold rounded-lg hover:bg-emerald-700">
                                    <span class="material-icons mr-1 text-base">add</span> Tambah KPI
                                </button>
                            </td>
                            <td class="px-4 py-2 text-center">
                                <span id="kpi-total-bobot" class="px-3 py-1 bg-blue-100 text-blue-700 rounded-lg font-extrabold">0%</span>
                            </td>
                ...
                <div class="flex justify-end mt-8">
                    <button type="button" class="btn-next bg-blue-600 text-white font-bold px-4 py-2 rounded-lg hover:bg-blue-700">Lanjut ke IKU <span class="material-icons text-sm ml-1 align-middle">arrow_forward</span></button>
                </div>
            </div>

            <div class="step-content hidden" data-step="2">
                <h2 class="text-2xl font-bold text-slate-800 mb-6">2. Indikator Kinerja Utama (IKU)</h2>
                <p class="text-slate-500 mb-4">Pilih IKU yang relevan dan tetapkan target kinerjanya.</p>
                        ...
                        <tbody id="iku-items" class="divide-y divide-slate-200">
                            ...
                            ?>
                                <tr data-index="<?php echo $currentIkuIndex; ?>" class="iku-item">
                                    <td class="px-4 py-2">
                                        <select name="iku_data[<?php echo $currentIkuIndex; ?>][iku_id]" class="w-full border-slate-300 rounded-md shadow-sm p-2" required>
                                    ...
                                    <td class="px-4 py-2">
                                        <input type="text" name="iku_data[<?php echo $currentIkuIndex; ?>][target]" value="<?php echo htmlspecialchars($item['target'] ?? ''); ?>" class="w-full text-center border-slate-300 rounded-md shadow-sm p-2 iku-target-input" placeholder="Cth: 100%" required>
                                    </td>
                                    <td class="px-4 py-2 text-center">
                        ...
                        <tfoot class="bg-slate-50">
                            <tr>
                                <td colspan="3" class="px-4 py-2 text-right">
                                    <button type="button" onclick="addItemIKU()" class="inline-flex items-center px-4 py-2 bg-emerald-600 text-white text-sm font-bold rounded-lg hover:bg-emerald-700">
                ...
                <div class="flex justify-between">
                    <button type="button" class="btn-prev bg-slate-400 text-white font-bold px-4 py-2 rounded-lg hover:bg-slate-500"><span class="material-icons text-sm mr-1 align-middle">arrow_back</span> Kembali</button>
                    <button type="button" class="btn-next bg-blue-600 text-white font-bold px-4 py-2 rounded-lg hover:bg-blue-700">Lanjut ke RAB <span class="material-icons text-sm ml-1 align-middle">arrow_forward</span></button>
                </div>
            </div>

            <div class="step-content hidden" data-step="3">
                <h2 class="text-2xl font-bold text-slate-800 mb-6">3. Rencana Anggaran Biaya (RAB)</h2>
                ...
                ?>
                <div class="mb-8 p-4 border border-slate-200 rounded-xl bg-slate-50/50">
                    <h3 class="text-xl font-bold text-slate-700 mb-3 flex items-center">
                        <span class="material-icons text-lg mr-2 text-blue-600">bookmark</span> <?= $catName ?>
                    </h3>
                    
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm mb-3 rab-table" data-category-id="<?= $catId ?>">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap min-w-[200px]">Uraian</th>
                                    <th class="px-2 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap w-[80px]">Vol 1</th>
                                    <th class="px-2 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap w-[100px]">Sat 1</th>
                                    <th class="px-1 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-[30px]"></th> <th class="px-2 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap w-[80px]">Vol 2</th>
                                    <th class="px-2 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap w-[100px]">Sat 2</th>
                                    <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap w-[120px]">Tot. Vol</th>
                                    <th class="px-1 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-[30px]"></th> <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap w-[150px]">Harga (Rp)</th>
                                    <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap w-[150px]">Total (Rp)</th>
                                    <th class="px-2 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap w-[50px]">Aksi</th>
                                </tr>
                            </thead>
                            <tbody id="rabBody_<?= $catId ?>" class="divide-y divide-slate-200">
                                <?php 
                                    ...
                                    <tr class="rab-row" data-index="<?php echo $rabGlobalIndex; ?>">
                                        <td class="px-4 py-2">
                                            <input type="text" name="rab_data[<?php echo $rabGlobalIndex; ?>][deskripsi]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm" value="<?php echo htmlspecialchars($item['deskripsi']); ?>" required>
                                        ...
                                        <td class="px-2 py-2">
                                            <input type="number" name="rab_data[<?php echo $rabGlobalIndex; ?>][volume_factor_1]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm text-right rab-volume-factor-1" value="<?php echo $vol1; ?>" min="0" step="0.01">
                                        </td>
                                        <td class="px-2 py-2">
                                            <select name="rab_data[<?php echo $rabGlobalIndex; ?>][satuan_factor_1_id]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm rab-satuan-factor-1" required onchange="toggleSatuanInput(this)">
                                            ...
                                            <input type="text" name="rab_data[<?php echo $rabGlobalIndex; ?>][satuan_factor_1_custom]" value="<?php echo htmlspecialchars($sat1_custom); ?>" class="satuan-custom-input w-full border-slate-300 rounded-md shadow-sm p-2 text-xs mt-1 <?php echo ($sat1_id !== 'NEW') ? 'hidden' : ''; ?>" placeholder="Nama Satuan">
                                        </td>
                                        
                                        <td class="px-1 py-2 text-center text-slate-400 font-bold">x</td>

                                        <td class="px-2 py-2">
                                            <input type="number" name="rab_data[<?php echo $rabGlobalIndex; ?>][volume_factor_2]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm text-right rab-volume-factor-2" value="<?php echo $vol2; ?>" min="0" step="0.01">
                                        </td>
                                        <td class="px-2 py-2">
                                            <select name="rab_data[<?php echo $rabGlobalIndex; ?>][satuan_factor_2_id]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm rab-satuan-factor-2" onchange="toggleSatuanInput(this)">
                                            ...
                                            <input type="text" name="rab_data[<?php echo $rabGlobalIndex; ?>][satuan_factor_2_custom]" value="<?php echo htmlspecialchars($sat2_custom); ?>" class="satuan-custom-input w-full border-slate-300 rounded-md shadow-sm p-2 text-xs mt-1 <?php echo ($sat2_id !== 'NEW') ? 'hidden' : ''; ?>" placeholder="Nama Satuan">
                                        </td>
                                        ...
                                        <td class="px-4 py-2">
                                            <input type="number" name="rab_data[<?php echo $rabGlobalIndex; ?>][harga_satuan]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm text-right rab-harga-satuan" value="<?php echo htmlspecialchars($item['harga_satuan']); ?>" min="0" step="0.01" required>
                                        </td>
                                        <td class="px-4 py-2 text-right">
                                            <span class="font-black text-slate-800 rab-total-display"><?= number_format_id($item['total']); ?></span>
                            ...
                            <tfoot>
                                <tr class="bg-slate-100 font-bold">
                                    <td colspan="9" class="px-4 py-2 text-right text-slate-600">Subtotal <?= $catName ?>:</td>
                                    <td colspan="2" class="px-4 py-2 text-right font-extrabold subtotal-display" data-cat-id="<?= $catId ?>">
                        ...
                        <button type="button" onclick="addRABRow(<?= $catId ?>)" class="inline-flex items-center px-4 py-2 bg-emerald-600 text-white text-sm font-bold rounded-lg hover:bg-emerald-700">
                ...
                <div class="mt-10 pt-6 border-t border-slate-200">
                    <h3 class="text-2xl font-bold text-slate-800 mb-4">Finalisasi & Simpan Usulan</h3>
                    
                    <div class="flex justify-between items-center mb-6 p-6 bg-blue-50 rounded-xl border border-blue-200 shadow-sm">
                        <div class="font-bold text-xl text-slate-800 uppercase tracking-wide">Total RAB:</div>
                        
                        <div class="flex items-baseline gap-2">
                            <span class="font-bold text-2xl text-slate-600">Rp</span>
                            <span id="finalGrandTotal" class="font-black text-4xl text-emerald-600 tracking-tight">
                    ...
                    <div class="flex justify-between">
                        ...
                        <div class="flex gap-4">
                            <button type="submit" name="action" value="draft" onclick="return validateDraft(event)" class="bg-amber-500 text-white font-bold px-6 py-3 rounded-lg shadow-md hover:bg-amber-600 transition-colors">
                                <span class="material-icons mr-2 text-base align-middle">save</span> Simpan sebagai Draft
                            </button>
                            
                            <button type="submit" name="action" value="ajukan" class="bg-blue-600 text-white font-bold px-6 py-3 rounded-lg shadow-lg hover:bg-blue-700 transition-colors" onclick="return validateSubmission(event)">
                                <span class="material-icons mr-2 text-base align-middle">send</span> Ajukan untuk Verifikasi
                            </button>
                        </div>
                    </div>
                </div>
    ...
    <template id="kpi-row-template">
        <tr class="kpi-item">
            <td class="px-4 py-2">
                <input type="text" name="indikator_kinerja[IDX][indikator]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm" placeholder="Contoh: Tercapainya 90% target peserta" required>
            </td>
            <td class="px-4 py-2">
                <select name="indikator_kinerja[IDX][bulan_target]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm kpi-month-select" required></select>
            </td>
            <td class="px-4 py-2">
                <input type="number" name="indikator_kinerja[IDX][bobot]" min="1" max="100" class="w-full text-center border-slate-300 rounded-md shadow-sm p-2 text-sm kpi-bobot-input" required>
            </td>
            <td class="px-4 py-2 text-center">
    ...
    <template id="iku-row-template">
        <tr class="iku-item">
            <td class="px-4 py-2">
                <select name="iku_data[IDX][iku_id]" class="w-full border-slate-300 rounded-md shadow-sm p-2" required>
                    <option value="">-- Pilih IKU --</option>
                </select>
            </td>
            <td class="px-4 py-2">
                <input type="text" name="iku_data[IDX][target]" class="w-full text-center border-slate-300 rounded-md shadow-sm p-2 iku-target-input" placeholder="Cth: 100%" required>
            </td>
            <td class="px-4 py-2 text-center">
                <button type="button" onclick="removeItemIKU(this)" class="text-rose-600 hover:text-rose-800 p-1">
                    <span class="material-icons text-lg">delete</span>
    ...
    <script>
        ...
        // --- VALIDASI DRAFT (Total RAB harus > 0) ---
        ...
        function addMultiInput(type) { 
            ...
            const itemHtml = `
                <div class="flex gap-2 ${type}-item">
                    <input type="text" name="${type}[]" value="" class="flex-1 px-3 py-2 border-slate-300 rounded-lg shadow-sm" placeholder="Contoh: Item Baru">
        ...
        function updateKpiBobotTotal() {
            ...
            if (total === 100) {
                totalSpan.className = 'px-3 py-1 bg-emerald-100 text-emerald-700 rounded-lg font-extrabold';
            } else {
                totalSpan.className = 'px-3 py-1 bg-rose-100 text-rose-700 rounded-lg font-extrabold';
            }
        }
        ...
        function addRABRow(catId) {
            ...
            const newRowHtml = `
                <tr class="rab-row" data-index="${rabGlobalIndex}">
                    <td class="px-4 py-2">
                        <input type="text" name="rab_data[${rabGlobalIndex}][deskripsi]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm" placeholder="Contoh: Konsumsi" required>
                    ...
                    <td class="px-2 py-2">
                        <input type="number" name="rab_data[${rabGlobalIndex}][volume_factor_1]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm text-right rab-volume-factor-1" value="" min="0" step="0.01">
                    </td>
                    <td class="px-2 py-2">
                        <select name="rab_data[${rabGlobalIndex}][satuan_factor_1_id]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm rab-satuan-factor-1" required onchange="toggleSatuanInput(this)">
                            ${document.getElementById('rab-satuan-options-template').innerHTML}
                        </select>
                        <input type="text" name="rab_data[${rabGlobalIndex}][satuan_factor_1_custom]" class="satuan-custom-input w-full border-slate-300 rounded-md shadow-sm p-2 text-xs mt-1 hidden" placeholder="Nama Satuan">
                    </td>

                    <td class="px-1 py-2 text-center text-slate-400 font-bold">x</td>

                    <td class="px-2 py-2">
                        <input type="number" name="rab_data[${rabGlobalIndex}][volume_factor_2]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm text-right rab-volume-factor-2" value="" min="0" step="0.01">
                    </td>
                    <td class="px-2 py-2">
                        <select name="rab_data[${rabGlobalIndex}][satuan_factor_2_id]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm rab-satuan-factor-2" onchange="toggleSatuanInput(this)">
                            ${document.getElementById('rab-satuan-options-template').innerHTML}
                        </select>
                        <input type="text" name="rab_data[${rabGlobalIndex}][satuan_factor_2_custom]" class="satuan-custom-input w-full border-slate-300 rounded-md shadow-sm p-2 text-xs mt-1 hidden" placeholder="Nama Satuan">
                    </td>
                    <td class="px-4 py-2 text-right">
                    ...
                    <td class="px-4 py-2">
                        <input type="number" name="rab_data[${rabGlobalIndex}][harga_satuan]" class="w-full border-slate-300 rounded-md shadow-sm p-2 text-sm text-right rab-harga-satuan" value="" min="0" step="0.01" required>
                    </td>
                    <td class="px-4 py-2 text-right">
                        <span class="font-black text-slate-800 rab-total-display"></span>
