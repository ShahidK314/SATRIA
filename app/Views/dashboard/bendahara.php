...
/**
 * Logika untuk menentukan status komprehensif (Pencairan & LPJ)
 */
function deriveDualStatus($row) {
    ...
    if ($totalCair < 1) {
        $s_cair = ['text' => 'Belum Dicairkan', 'class' => 'bg-blue-100 text-blue-700'];
    } elseif ($isFullyCair) {
        $s_cair = ['text' => 'Pencairan Selesai', 'detail' => $percentage . '%', 'class' => 'bg-emerald-100 text-emerald-700'];
    } else {
        $s_cair = ['text' => 'Cair Bertahap', 'detail' => $percentage . '%', 'class' => 'bg-cyan-100 text-cyan-700'];
    }
    ...
    if ($totalCair >= 1) {
        if ($lpjStatus === 'Disetujui') {
            $s_lpj = ['text' => 'Selesai/Disetujui', 'class' => 'bg-slate-800 text-white'];
        } elseif ($lpjStatus === 'Revisi') {
            $s_lpj = ['text' => 'LPJ Revisi', 'class' => 'bg-rose-100 text-rose-700 animate-pulse'];
        } elseif ($lpjStatus === 'Diajukan') {
            $s_lpj = ['text' => 'Diajukan (Verifikasi)', 'class' => 'bg-violet-100 text-violet-700'];
        } elseif ($lpjStatus === 'Belum Upload' && $isOverdue) {
            $s_lpj = ['text' => 'TELAT LPJ!', 'class' => 'bg-rose-600 text-white animate-pulse'];
        } elseif ($lpjStatus === 'Belum Upload') {
            $s_lpj = ['text' => 'Wajib Upload', 'class' => 'bg-amber-100 text-amber-700'];
        }
    }
...
<div class="m-5">
    <div class="mb-8">
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Dashboard Bendahara</h1>
        <p class="text-slate-500 mt-1">Selamat datang, <?php echo htmlspecialchars($_SESSION['username'] ?? 'Bendahara'); ?>. Ringkasan Keuangan Sistem.</p>
    </div>
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6">
        <form method="GET" class="flex flex-col md:flex-row gap-4 items-end">
            ...
            <div class="flex-1 min-w-[200px] w-full">
                ...
                <div class="relative">
                    ...
                    <input
                        ...
                        class="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm
                            leading-5 focus:ring-2 focus:ring-blue-500 outline-none"
                    >
                </div>
            </div>
                ...
                <select name="pengusul"
                    ...
                    <?php if (isset($listPengusul)): foreach ($listPengusul as $p): ?>
                        <option value="<?= $p['username'] ?>"
                            <?= ($filterProposer ?? '') == $p['username'] ? 'selected' : '' ?>>
            ...
            <div class="flex gap-2 w-full md:w-auto">
                <button type="submit"
                    class="flex-1 md:flex-none px-6 py-2 bg-slate-800 text-white font-bold rounded-lg
                        hover:bg-slate-900 transition-all text-sm">
                    Filter
                </button>
                <a href="/dashboard"
                    class="flex-1 md:flex-none px-6 py-2 bg-slate-100 text-slate-600 font-bold rounded-lg
                        hover:bg-slate-200 transition-all text-sm text-center">
                    Reset
                </a>
            </div>
    ...
    <div class="mt-8">
        <h2 class="text-xl font-bold text-slate-800 mb-4">Progres Semua Kegiatan yang Disetujui WD2</h2>
                        ...
                        <?php else: ?>
                            ...
                            <tr class="hover:bg-slate-50 transition-colors <?= $isOverdue ? 'bg-rose-50/40' : '' ?>">
                                <td class="px-6 py-4">
                                    ...
                                    <p class="text-[10px] text-slate-400 uppercase tracking-wider">Oleh: <?php echo htmlspecialchars($row['username']); ?></p>
                                </td>
                                <td class="px-6 py-4 text-right text-emerald-600 font-bold font-mono">
                                    <?php echo formatRupiah($row['nominal_pencairan']); ?>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="px-3 py-1 rounded text-[10px] font-bold <?= $dualStatus['cair']['class']; ?>">
                                        <?php echo $dualStatus['cair']['text']; ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="px-3 py-1 rounded text-[10px] font-bold <?= $dualStatus['lpj']['class']; ?>">
                                        <?php echo $dualStatus['lpj']['text']; ?>
                                    </span>
                                </td>
