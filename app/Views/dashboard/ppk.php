<?php 
// app/Views/dashboard/ppk.php (FINALIZED: Displaying Counts: Total Flow, Pending, Approved)
...
// FIX: Inisialisasi usulan jika kosong
$usulan = $usulan ?? [];

// 1. Dapatkan hitungan Menunggu PPK
$menungguPPK = count($usulan); 
// 2. Dapatkan hitungan Disetujui PPK, nunggu WD2 (Data yang sudah disetujui PPK, nunggu step berikutnya)
$pendingWD2 = $stats['menunggu_wd2'] ?? 0; 
// 3. Dapatkan hitungan Disetujui PPK, sudah disetujui WD2 (Data yang sudah melewati PPK dan WD2)
$approvedTotal = $stats['disetujui_total'] ?? 0;

// Hitungan Total Disetujui (Total kegiatan yang sudah melewati PPK)
$disetujuiTotal = $pendingWD2 + $approvedTotal; 

// Hitungan Total Kegiatan di Alur PPK (Sinkron: Menunggu PPK + Disetujui Total)
$totalApprovalFlow = $menungguPPK + $disetujuiTotal; 
?>

<div class="m-5">
    <div class="mb-8">
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Dashboard Pejabat Pembuat Komitmen (PPK)</h1>
        <p class="text-slate-500 mt-1">Selamat datang, <?php echo htmlspecialchars($_SESSION['username'] ?? 'PPK'); ?>. Ringkasan kegiatan dan antrian persetujuan Anda.</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-slate-200 p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl font-bold text-slate-800 flex items-center">
                <span class="material-icons text-lg mr-2 text-blue-600">assignment</span> Daftar Kegiatan Menunggu Persetujuan
            </h2>
        </div>
        ...
        <div class="space-y-4">
            ...
            <div class="flex justify-between items-center p-3 border border-slate-100 rounded-lg hover:bg-blue-50 transition-colors">
                ...
                <a href="/pengajuan/ppk/detail?id=<?php echo $r['id']; ?>" class="px-3 py-1 bg-blue-600 text-white text-xs font-bold rounded-lg hover:bg-blue-700">
