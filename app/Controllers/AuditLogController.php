<?php
// app/Controllers/AuditLogController.php (UPDATED: Allow Direktur Read Access)
...
class AuditLogController
{
    ...
    private function checkAccess() {
        // [FIX] Izinkan Admin DAN Direktur untuk melihat log (sebelumnya hanya Admin)
    ...
    public function export()
    {
        ...
        // FIX: Hapus output buffer untuk mencegah header error saat export CSV
        if (ob_get_level() > 0) {
            ob_end_clean();
        }
        ...
        foreach ($logs as $log) {
            // Pastikan data di-encode dengan benar ke array untuk fputcsv
            fputcsv($output, array_values($log));
        }
